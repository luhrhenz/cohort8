...
describe('TimeLock', function () {
  ...
  describe('Deposits', function () {
    ...
    it('Should revert if deposit is 0', async function () {
      ...
      await expect(
        timelock.deposit(unlockTime, { value: 0 })
    ...
    it('Should revert if unlock time is in the past', async function () {
      ...
      await expect(
        timelock.deposit(pastTime, { value: DEPOSIT_AMOUNT })
  ...
  describe.only('Withdrawals', function () {
    it('Should fail if funds are still locked', async function () {
      ...
      await expect(timelock.withdraw(0)).to.be.revertedWith(
  ...
  describe('WithdrawAll', function () {
    it('Should withdraw from multiple unlocked vaults at once', async function () {
      ...
      await timelock.deposit(now + ONE_DAY_IN_SECS * 2, {
        value: DEPOSIT_AMOUNT,
      }); // Vault 1
      await timelock.deposit(now + ONE_DAY_IN_SECS * 10, {
        value: DEPOSIT_AMOUNT,
      }); // Vault 2 (remains locked)
      ...
      await expect(timelock.withdrawAll()).to.changeEtherBalance(
        ethers,
        owner,
        expectedTransfer
      );

      expect(await timelock.getTotalBalance(owner.address)).to.equal(
        DEPOSIT_AMOUNT
      ); // Only Vault 2 remains
    });
    ...
    it('Should revert if no vaults are ready for withdrawal', async function () {
      ...
      await timelock.deposit(unlockTime, { value: DEPOSIT_AMOUNT });
      await expect(timelock.withdrawAll()).to.be.revertedWith(
        'No unlocked funds available'
      );
    });
  });
  ...
  describe('View Functions', function () {
    it('Should correctly track total and unlocked balances', async function () {
      ...
      expect(await timelock.getUnlockedBalance(owner.address)).to.equal(
        ethers.parseEther('1')
      );
    });
